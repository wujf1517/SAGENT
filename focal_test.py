...
def create_dataset(path,batch_size,epoch_num):
    ...
    dataset = dataset.shuffle(buffer_size=2000).batch(batch_size).repeat(epoch_num)  
    return dataset
...
'''标签三个，原模型标签两个，需要修改吗？'''
# def binary_focal_loss_fixed(logits, y_true,gamma=2, alpha=0.25):
