...
if num_image_1>num_image_2:
    ...
    raw_rate = (num_image_2)/(num_image_1+num_image_2)
else :
    num_image_weight = (num_image_2)/num_image_1
    raw_rate = (num_image_1)/(num_image_1+num_image_2)
print(num_image_weight)
bais_last_layer = -math.log10((1-raw_rate)/raw_rate)
...
def create_dataset(path,batch_size,epoch_num):
    ...
    dataset = dataset.shuffle(buffer_size=2000).batch(batch_size).repeat(epoch_num)  # shuffle(buffer_size=800) 取800个图片并打乱
    return dataset
...
'''标签三个，原模型标签两个，需要修改吗？'''
# def binary_focal_loss_fixed(logits, y_true,gamma=1.5, alpha=0.25):
...
#     y_pred = tf.clip_by_value(probs, epsilon, 1. - epsilon) # 输入一个张量A，把A中的每一个元素的值都压缩在min和max之间。小于min的让它等于min，大于max的元素的值等于max。
#     # y_pred = logits
#     # 得到调节因子weight和alpha
...
def bias_variable_lastlayer(shape):
    # 
    initial = tf.constant(-2.0, shape=shape)  # 原来为tf.constant(0.0, shape=shape)
    return tf.Variable(initial)
...
with tf.name_scope('fc2_layer'):
    ...
    b_fc2 = bias_variable_lastlayer([num_classes])  # 分类数目改了，这里偏置量也得改由2改为3
    y_conv = tf.nn.softmax(tf.matmul(h_fc1_drop, W_fc2) + b_fc2,name='y_conv') # 输出归一化
...
with tf.Session() as sess:
    ...
    batch_idxs = int(0.9*num_examples/batch_size) # 设置迭代次数 int(0.3*num_examples/batch_size)
