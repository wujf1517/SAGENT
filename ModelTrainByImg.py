 ...
 * @time: 22:32:59 
 * Version:1.0.0
 * description: when i train this model by tfrecord file,some problem had  showed:build a tfrecord file by same image
...
def create_dataset(path,batch_size,epoch_num):
    ...
    dataset = dataset.shuffle(buffer_size=800).batch(batch_size).repeat(epoch_num)  # shuffle(buffer_size=800) 取800个图片并打乱
    return dataset

dataset = create_dataset(path,batch_size,epoch+1)   # 图片所在的路径为path
...
with tf.Session() as sess:
    ...
    batch_idxs = int(0.3*num_examples/batch_size) # 设置迭代次数
